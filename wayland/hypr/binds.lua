...
hl.bind(
  mainMod .. " + D",
  hl.dsp.exec_cmd("/storage/src/dot_files/wayland/scripts/hyprland-cheatsheet.sh"),
  { description = "this cheatsheet" }
)
hl.bind(
  mainMod .. " + H",
  hl.dsp.exec_cmd('kitty --title "Commands Cheatsheets" -o confirm_os_window_close=0 ~/scripts/cheats-commands.sh'),
  { description = "commands cheatsheets" }
)
hl.bind(mainMod .. " + SHIFT + J", hl.dsp.exec_cmd("pypr wall next"), { description = "next wallpaper" })
hl.bind(
  mainMod .. " + SHIFT + K",
  hl.dsp.exec_cmd(
    "loupe /storage/src/dot_files/mechanical-keyboards/corne-v4/images/split-keyboard-corne-v4-layers.png"
  ),
  { description = "corne v4 split keyboard layout" }
)
hl.bind(mainMod .. " + Q", hl.dsp.exec_cmd("~/scripts/powermenu-hypr.sh"), { description = "power menu" })
hl.bind(
  mainMod .. " + R",
  hl.dsp.exec_cmd("~/scripts/wayland-reload.sh"),
  { description = "reload hyprland/pyprland/waybar" }
)
hl.bind(
  mainMod .. " + T",
  hl.dsp.exec_cmd("/storage/src/dot_files/wayland/scripts/tmux-cheatsheet.sh"),
...
hl.bind(
  mainMod .. " + C",
  hl.dsp.exec_cmd("cliphist list | wofi --show dmenu | cliphist decode | wl-copy"),
  { description = "select entry from clipboard" }
)
hl.bind(
  mainMod .. " + E",
  hl.dsp.exec_cmd("vicinae toggle"),
  { description = "open applications/clipboard/etc.. (vicinae)" }
)
hl.bind(
  mainMod .. " + O",
  hl.dsp.exec_cmd("/storage/src/dot_files/wayland/scripts/screen-record.sh"),
  { description = "record/capture video from selected screen area" }
)
hl.bind(
  mainMod .. " + P",
  hl.dsp.exec_cmd("wayscriber --active"),
  { description = "take screenshot (F2 toggle floating panels, Ctrl+Shift+S select region)" }
)
...
hl.bind(
  "CTRL + Period",
  hl.dsp.exec_cmd('kitty --title "Snippet Selector" ~/scripts/snippets-fzf.sh'),
  { description = "text snippets fzf" }
)
hl.bind(
  mainMod .. " + A",
  hl.dsp.exec_cmd("~/scripts/browser-bookmarks.sh"),
  { description = "copy browser bookmark to clipboard" }
)
hl.bind(mainMod .. " + B", hl.dsp.exec_cmd("~/scripts/pass-menu.sh"), { description = "decrypt password (pass)" })
hl.bind(
  mainMod .. " + M",
  hl.dsp.exec_cmd(
    'kitty --title "AI Prompt Popup" -o confirm_os_window_close=0 /storage/src/dot_files/wayland/scripts/ai-prompt-popup.sh'
  ),
  { description = "AI prompt select" }
)
...
hl.bind(
  mainMod .. " + SHIFT + right",
  hl.dsp.layout("mfact -0.1"),
  { description = "window: shrink master split size" }
)
hl.bind(mainMod .. " + TAB", hl.dsp.focus({ last = true }), { description = "window: focus last window" })
hl.bind(
  "ALT + TAB",
  hl.dsp.exec_cmd("~/scripts/hyprland-go-to-window.sh"),
  { description = "window: select from menu" }
)
...
hl.bind(
  ...
  { description = "window: move current to workspace 1 and switch to it" }
)
hl.bind(
  mainMod .. " + SHIFT + 2",
  hl.dsp.window.move({ workspace = 2 }),
  { description = "window: move current to workspace 2 and switch to it" }
)
hl.bind(
  mainMod .. " + SHIFT + 3",
  hl.dsp.window.move({ workspace = 3 }),
  { description = "window: move current to workspace 3 and switch to it" }
)
hl.bind(
  mainMod .. " + SHIFT + 4",
  hl.dsp.window.move({ workspace = 4 }),
  { description = "window: move current to workspace 4 and switch to it" }
)
hl.bind(
  mainMod .. " + SHIFT + 5",
  hl.dsp.window.move({ workspace = 5 }),
  { description = "window: move current to workspace 5 and switch to it" }
)
hl.bind(
  mainMod .. " + SHIFT + 6",
  hl.dsp.window.move({ workspace = 6 }),
  { description = "window: move current to workspace 6 and switch to it" }
)
hl.bind(
  mainMod .. " + SHIFT + 7",
  hl.dsp.window.move({ workspace = 7 }),
  { description = "window: move current to workspace 7 and switch to it" }
)
hl.bind(
  mainMod .. " + SHIFT + 8",
  hl.dsp.window.move({ workspace = 8 }),
  { description = "window: move current to workspace 8 and switch to it" }
)
hl.bind(
  mainMod .. " + SHIFT + 9",
  hl.dsp.window.move({ workspace = 9 }),
  { description = "window: move current to workspace 9 and switch to it" }
)
hl.bind(
  mainMod .. " + SHIFT + 0",
  hl.dsp.window.move({ workspace = 10 }),
  { description = "window: move current to workspace 10 and switch to it" }
)
...
hl.bind(
  ...
  { description = "window: move current to workspace 1 and DO NOT switch to it" }
)
hl.bind(
  mainMod .. " + ALT + 2",
  hl.dsp.window.move({ workspace = 2, follow = false }),
  { description = "window: move current to workspace 2 and DO NOT switch to it" }
)
hl.bind(
  mainMod .. " + ALT + 3",
  hl.dsp.window.move({ workspace = 3, follow = false }),
  { description = "window: move current to workspace 3 and DO NOT switch to it" }
)
hl.bind(
  mainMod .. " + ALT + 4",
  hl.dsp.window.move({ workspace = 4, follow = false }),
  { description = "window: move current to workspace 4 and DO NOT switch to it" }
)
hl.bind(
  mainMod .. " + ALT + 5",
  hl.dsp.window.move({ workspace = 5, follow = false }),
  { description = "window: move current to workspace 5 and DO NOT switch to it" }
)
hl.bind(
  mainMod .. " + ALT + 6",
  hl.dsp.window.move({ workspace = 6, follow = false }),
  { description = "window: move current to workspace 6 and DO NOT switch to it" }
)
hl.bind(
  mainMod .. " + ALT + 7",
  hl.dsp.window.move({ workspace = 7, follow = false }),
  { description = "window: move current to workspace 7 and DO NOT switch to it" }
)
hl.bind(
  mainMod .. " + ALT + 8",
  hl.dsp.window.move({ workspace = 8, follow = false }),
  { description = "window: move current to workspace 8 and DO NOT switch to it" }
)
hl.bind(
  mainMod .. " + ALT + 9",
  hl.dsp.window.move({ workspace = 9, follow = false }),
  { description = "window: move current to workspace 9 and DO NOT switch to it" }
)
hl.bind(
  mainMod .. " + ALT + 0",
...
hl.bind(
  mainMod .. " + W",
  hl.dsp.exec_cmd("~/scripts/hyprland-move-current-workspace.sh"),
  { description = "workspace: move current to other monitor" }
)
hl.bind(
  mainMod .. " + SHIFT + W",
  hl.dsp.exec_cmd("~/scripts/hyprland-move-selected-workspace.sh"),
  { description = "workspace: move selected to other monitor" }
)
...
hl.bind(
  ...
  { description = "workspace: mouse scroll up to next" }
)
hl.bind(
  mainMod .. " + mouse_down",
  hl.dsp.focus({ workspace = "e-1" }),
